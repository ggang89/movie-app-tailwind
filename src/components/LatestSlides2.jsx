...
export default function LatestSlides2() {
  const [loading, setLoading] = useState(true);
  const [ratingMovies, setRatingMovies] = useState([]);
  const getRatingMovies = async () => {
    const response = await fetch(
      "https://yts.mx/api/v2/list_movies.json?sort_by=year&&limit=7"
    );
          ...
          {ratingMovies.map((m) => (
            ...
            >
              <div className="w-[150px] h-[300px] flex flex-wrap justify-center ">
                <Movies img={m.medium_cover_image} title={m.title} runtime={m.runtime} rating={m.rating}/>
              </div>
            </div>
          ))}
        </div>
